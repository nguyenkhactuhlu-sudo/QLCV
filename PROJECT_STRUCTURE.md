...
> Tài liệu đọc nhanh dành cho lập trình viên hoặc LLM tiếp quản dự án. Cập nhật gần nhất: 06/09/2026.
...
> **Thêm 06/09/2026** (theo phản ánh thực tế thí điểm tại Khu vực 1, xem `Kich_ban_demo_QLCV_don_vi_thi_diem.docx`/lịch sử commit để biết chi tiết): sửa lỗi Trưởng phòng/Viện trưởng khu vực có thể duyệt nhầm nhật ký đã nộp đích danh cho 1 Phó (tách 2 khu trong "Duyệt & chấm điểm"); Ghi chú công việc có nút "Ghi nhật ký cho việc này" + nhắc hạn theo giờ; ô chọn "người phối hợp" trong Giao việc làm lại kiểu thẻ/nhóm; thêm cách xem "Theo ngày" trong Nhật ký công tác của đơn vị (ai đã nộp/nghỉ phép/chưa nộp); thêm mục "Điểm cộng/trừ đột xuất" (bảng `score_adjustments` mới, migration 00065-00066) cho khen thưởng/kỷ luật phát hiện sau khi tháng đã chấm xong, không sửa lại điểm tháng đã chốt. Cả 6 việc trên đã làm cho **cả `production/` lẫn `demo/`** (demo mô phỏng bằng mảng `scoreAdjustments` + `localStorage`, không cần Supabase).
...
├── supabase/
│   ├── config.toml
│   ├── migrations/            # 00001 → 00066 (+ 1 file gộp tiện dụng), xem mục 7
